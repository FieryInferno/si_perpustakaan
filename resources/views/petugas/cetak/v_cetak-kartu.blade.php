...
<body>
  <div class="container">
    ...
    <div class="profile">
      <img class="profil" src="{{ asset('img/img-anggota/' . $anggota->gambar)}}" alt="">
    </div>
    <div class="judul">
      <label>Kartu Anggota <br> SI-Perpus</label>
    </div>
    <div class="status" style="background-color: orange; width:100px;">
      <label>{{$anggota->status}}</label>
    </div>
    <div class="nama">
      <label for="nama">{{$anggota->nama_anggota}}</label>
    </div>
    <!-- <div class="tgl_create">
      <label for="tgl_create">
        <?php 
          $Format = strtotime($anggota->created_at);
